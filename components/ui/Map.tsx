      ...
      {store.storeInfo.map((storeInfo) => {
        return isOpen.value && (
          <div>
            ...
            <button
              class="uppercase mt-6 flex justify-center items-center border border-solid border-primary-600 w-full gap-2 p-2"
              onClick={() => {
                storeLocal.value = {
...
function Stores({ stores }: { stores: StoreProps[] }) {
  return (
    <div class="absolute top-1/3 bg-secondary-neutral-100 py-8 px-10 left-8 min-w-[480px]">
      <h2 class="text-dark-blue uppercase mb-8 text-2xl">Nossas Lojas</h2>
